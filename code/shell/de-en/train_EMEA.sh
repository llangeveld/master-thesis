#!/bin/bash
START=/data/s3225143
DATA_EMEA=$START/data/4_translation/processed/EMEA.de-en/
DATA_JRC=$START/data/4_translation/processed/JRC.de-en/
DATA_GNOME=$START/data/4_translation/processed/GNOME.de-en/
MODELS=$START/models
PRETRAINED=$MODELS/wmt19.de-en.joined-dict.ensemble/model1.pt
...
CUDA_VISIBLE_DEVICES=0 fairseq-train $DATA_EMEA \
  ...
  --max-update 42200 --update-freq 1 \
  ...
  --bpe fastbpe --bpe-codes /data/s3225143/models/wmt19.de-en.joined-dict.ensemble/bpecodes \
  --tokenizer moses \
